    ...
    $(window).resize(function(){
        ...
        $('.m-menu').click(function(){
            if(boolean){
                ...
                $('.line-wrap span:first-child').css({"top" : "50%","transform" : "translateY(-50%)"});
                $('.line-wrap span:nth-of-type(2)').css("opacity",0);
                $('.line-wrap span:last-child').css({"top" : "50%","transform" : "translateY(-50%)"});
                $('.m-nav-list').css("left","0")
                setTimeout(()=>{
                    $('.line-wrap span:first-child').css({"top" : "50%","transform" : "translateY(-50%) rotate(45deg)"});
                    $('.line-wrap span:last-child').css({"top" : "50%","transform" : "translateY(-50%) rotate(-45deg)"});
                },250)
                boolean = false;
            }else{
                ...
                $('.line-wrap span:first-child').css({"top" : "50%","transform" : "translateY(-50%)"});
                $('.line-wrap span:last-child').css({"top" : "50%","transform" : "translateY(-50%)"});
                $('.m-nav-list').css("left","100%")
                setTimeout(()=>{
                    $('.line-wrap span:nth-of-type(2)').css("opacity",1);
                $('.line-wrap span:first-child').css({"top" : "0","transform" : ""});
                $('.line-wrap span:last-child').css({"top" : "100%","transform" : "translateY(-100%)"});
                },250)
                boolean = true;
            }
        })
        ...
        for(let i = 0; i<235; i++)
        {
        let imgBox = `<img class="main main${i}" src="./img/movie/${i}.png" alt="">`
        ...
        let [imgNum,a,b,c,d,e,scaleA,scaleB,scaleC,scaleD,scaleE] = [];
            imgNum = parseInt(scrollY / 30);
            imgNum > 236 ? imgNum = 235 : imgNum = imgNum;
        ...
        $('.main'+imgNum).css("visibility","visible");
            function tshirtAnimateM(){
                let twrapRatio = (scrollY + $(window).innerHeight()*0.35 - $('.t-shirt-wrap').offset().top) / $('.t-shirt-wrap').innerHeight();
        ...
        // -------Goods T-shirt scroll animation-------//
            ...
            for(let i = 0; i<5; i++){
                ...
                if(scrollY > 9000 && scrollY <9200){
                    let opa= (scrollY - 9000) / 200
                    let trans = (9200 - scrollY ) / 4;

                    $('.t-demon1').css("transform", `translateY(${trans}px)`)
                                    .css("opacity",opa);
                }
                if(scrollY > 9200 && scrollY <9400){
                    let opa= (scrollY - 9200) / 200
                    let trans = (9400 - scrollY ) / 4;
...
//------------------observer-------------------//
    ...
    let observerLR = new IntersectionObserver((e)=>{
        e.forEach((a)=>{
            if(a.isIntersecting){
                    a.target.style.opacity = 1;
                    a.target.style.left = "-100px"
            } else{
                    a.target.style.opacity = 0;
                    a.target.style.left = "-190px"
            }
        })
    })
    let observerRL = new IntersectionObserver((e)=>{
        e.forEach((a)=>{
            if(a.isIntersecting){
                    a.target.style.opacity = 1;
                    a.target.style.transform = "translateX(0px)"
                 
            } else{
                    a.target.style.opacity = 0;
                    a.target.style.transform = "translateX(100px)"
            }
        })
    })
    let observerLR2 = new IntersectionObserver((e)=>{
        e.forEach((a)=>{
            if(a.isIntersecting){
                    a.target.style.opacity = 1;
                    a.target.style.transform = "translateX(0px)"
    ...
    let observerTB = new IntersectionObserver((e)=>{
        let delaytime = 0;
        e.forEach((a)=>{
    ...
    let observerTB2 = new IntersectionObserver((e)=>{
        let delaytime = 0;
        e.forEach((a)=>{
    ...
    let ob = [
        [observerBT,bottomTotop],
        [observerLR,leftToright],
        [observerRL,rightToleft],
        [observerLR2,leftToright2],
        [observerTB,topTobottom]
    ]
    ...
    function obf1(){
        ob.forEach((a)=>{
        a[1].forEach((e)=>{
        a[0].observe(document.querySelector(`${e}`)) 
